@extends('layouts.app', ['class' => 'off-canvas-sidebar', 'activePage' => 'login', 'title' => __('Olho Gordo - Admin')])
...
{{--            <p class="card-description text-center">{{ __('Or Sign in with ') }} <strong>admin@material.com</strong> {{ __(' and the password ') }}<strong>secret</strong> </p>--}}
            ...
            <div class="bmd-form-group{{ $errors->has('password') ? ' has-danger' : '' }} mt-3">
              <div class="input-group">
                ...
                <input type="password" name="password" id="password" class="form-control" placeholder="{{ __('Senha...') }}" value="{{ !$errors->has('password') ? "secret" : "" }}" required>
              </div>
